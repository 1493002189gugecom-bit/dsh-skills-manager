...
// ---------------------------------------------------------------------------
/**
 * Skill center panel (browser half): an overlay modal with three tabs — the
 * grouped skill list (enable/disable switch, delete), a create form, and the
 * directory manager added in this fork. Talks to the host route family through
 * SkillApi.
 */
...
type Tab = 'list' | 'create' | 'directories'
    ...
    >
      <div className={css.card} data-dsh-part="card">
        ...
        <div className={css.tabs} data-dsh-part="tab-bar" role="tablist">
          ...
          <button type="button" role="tab" className={`${css.tab} ${tab === 'directories' ? css.tabActive : ''}`} data-dsh-part="tab" aria-selected={tab === 'directories'} data-active={tab === 'directories' ? '' : undefined} onClick={() => { setTab('directories') }}>
            {tt('tab.directories')}
          </button>
        </div>
        <div className={css.body}>
          {tab === 'list' && <ListTab api={api} refreshTick={refreshTick} onCwd={setCwd} />}
          {tab === 'create' && <CreateTab api={api} cwd={cwd} />}
          {tab === 'directories' && (
            <DirectoriesTab api={api} onChanged={() => { setRefreshTick((tick) => tick + 1) }} />
          )}
        </div>
      </div>
    </div>
