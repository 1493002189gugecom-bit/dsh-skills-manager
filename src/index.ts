 ...
 * /api/dsh-skills-manager route family (list grouped by source, set enabled,
 ...
 *
 * Ported from the local dsh-skills-manager plugin; everything rides official
...
function applyImpl(ctx: Context, config?: Config): void {
  ...
  const routes = makeRoutes(ctx, {
    dshHome,
    agentsHome,
    // Config roots plus whatever the panel has added, resolved once at apply
    // time; the routes re-read the store per request so panel changes land
    // without a restart.
    customSkillDirs: listDirectories(dshHome, customSkillDirs),
 ...
 */
export const apply = mountOnce('dsh-skills-manager', applyImpl)
